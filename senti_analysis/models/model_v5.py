...
def get_model(learning_rate=config.LEARNING_RATE, name='model_v4'):
    ...
    share_hidden = tf.keras.layers.Conv1D(filters, kernel_size, padding='valid',
                                          activation='relu', strides=1)(embedding)
    share_hidden = tf.keras.layers.MaxPooling1D(pool_size=pool_size)(share_hidden)
    share_hidden = tf.keras.layers.GRU(256, activation='relu', return_sequences=True, reset_after=True)(share_hidden)
